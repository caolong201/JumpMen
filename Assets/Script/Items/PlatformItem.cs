			...
			} else 
			{
				...
				if (rand >= 3) {
					...
					if (rand >= 6) {//pos 1
						...
						if (rand == 6) { //truong hop nay randdom itemchangecamera
						...
						if (rand == 3) { //truong hop nay randdom itemchangecamera
							int r = UnityEngine.Random.Range (0, 5);
							if (r == 1) {
								GameObject itemChange = Instantiate (itemChangeCamra) as GameObject;
								itemChange.transform.SetParent (gameObject.transform);
				...
				} else if (rand == 1) {//truong hop nay random item bonus
					if (PlayerState.Instance.playerCurrState != State.AutoRun) {// neu dang chay auto thi hk tao nua
						int r = UnityEngine.Random.Range (0, 5);
						if (r == 1) {
